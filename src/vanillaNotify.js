...
var vNotify = (function() {
    ...
    var addNotify = function(params) {
        ...
        item.fadeInterval = fade('in', item.options.fadeInDuration, item);
    ...
    var fade = function(type, ms, el) {
        ...
        function func() {
            ...
            if(opacity <= 0) {
                if (el) {
                    window.clearInterval(el.fadeInterval);
                    remove(el);
                }
...
var v = {
    positionOption: vn.positionOption,
    options: vn.options,
    notifications: {
        info: vn.info,
        success: vn.success,
